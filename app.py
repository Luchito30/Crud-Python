...
with app.app_context():
    ...
    # Verificar si existe el usuario 'Murdoy'
    existing_user = Usuario.query.filter_by(user='Murdoy').first()
    if not existing_user:
        # Crear el nuevo usuario 'Murdoy'
...
def get_usuario(id):
    try:
        ...
        if usuario:
            # Corrección en el nombre del campo de contraseña
            return jsonify({
                "id": usuario.id,
                "usuario": usuario.user,
                "password": usuario.password,  # Corrección en el nombre del campo de contraseña
...
# Endpoint POST para la creación de usuarios
...
def crear_usuario():
    try:
        ...
        nuevo_usuario = Usuario(user=usuario, password=hashed_password, rol_id=rol_id)

        # Agregar el nuevo usuario a la sesión y realizar la confirmación
        db.session.add(nuevo_usuario)
        db.session.commit()

        # Devolver la respuesta JSON con el nuevo usuario y el código de estado 201
        return jsonify({'mensaje': 'Usuario creado exitosamente'}), 201

    except ValidationError as err:
        # Manejar errores de validación usando marshmallow
        return jsonify({'error': err.messages}), 400

    except Exception as e:
        # Manejar otros errores en el servidor
        return jsonify({'error': f'Error en el servidor: {str(e)}'}), 500
...
def update_usuario(id):
    try:
        ...
        if usuario:
            ...
            # Corrección en el nombre del campo de contraseña
            usuario.user = json_data.get('usuario', usuario.user)

            # Verificar si hay una nueva contraseña
            nueva_clave = json_data.get('clave')
            if nueva_clave:
                # Hashear la nueva contraseña con bcrypt
                hashed_clave = bcrypt.generate_password_hash(nueva_clave).decode('utf-8')
                usuario.password = hashed_clave

            usuario.rol_id = json_data.get('rol', usuario.rol_id)
...
# Ruta para eliminar un usuario mediante una solicitud DELETE
...
def delete_usuario(id):
    try:
        ...
        if usuario:
            ...
            return jsonify({'error': 'Usuario no encontrado'}), 404  # Devolver código 404 si el usuario no existe
